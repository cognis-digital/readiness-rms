...
**readiness-rms** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `readiness-rms` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
readiness-rms ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
readiness-rms ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
readiness-rms ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `readiness-rms` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="readiness-rms") for rec in records]   # records = your JSON output
