...
_SKIP = {"target", "format", "out", "fail_on", "version", "classification"}
...
def make_cli(tool_name, scan_fn, version="0.1.0", extra_args=None):
    ...
    if extra_args:
        for a in extra_args:
            flags = a["flags"]
            kwargs = {k: v for k, v in a.items() if k != "flags"}
            p.add_argument(*flags, **kwargs)
    args = p.parse_args()
    try:
        extra_kwargs = {k: v for k, v in vars(args).items() if k not in _SKIP}
        result = scan_fn(args.target, **extra_kwargs)
    except SystemExit:
        raise
    ...
    if args.out:
        try:
            ...
            print(
                f"error: cannot write output file '{args.out}': {exc}",
                file=sys.stderr,
            )
            sys.exit(2)
