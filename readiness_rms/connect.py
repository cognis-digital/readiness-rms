"""Native cognis-connect emit for readiness-rms — forward findings to any platform.

Maps readiness-rms's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    readiness-rms ... --format json | readiness-rms-emit --to stix
    readiness-rms-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('id', '')
        out['severity'] = rec.get('rating', 'info')
        out['type'] = 'finding'
        out['description'] = rec.get('message', '')
